...
# Added 0 as the default first option
...
if menu == "Dashboard":
    ...
    if not sales_df.empty:
        ...
        st.divider()
        st.subheader("Payment Breakdown (Today)")
        ...
        c1, c2 = st.columns(2)
        c1.info(f"**Cash:** ₱{cash:,.2f}")
        c2.info(f"**GCash:** ₱{gcash:,.2f}")
    else:
        st.info("No data yet.")

# 2. NEW SALE (DROPDOWN ADD-ONS)
elif menu == "New Sale":
    ...
    with st.form("order_form", clear_on_submit=True):
        ...
        with col2:
            ...
            # UPDATED: Add-on options now in a dropdown
        ...
        notes = st.text_area("Notes / Remarks (e.g., 'Extra bleach', 'Delivery fee')")
        work_status = st.select_slider("Initial Work Status", options=["WIP", "Ready", "Claimed"])

        # Calculation Logic
        base_price = TIERS[selected_tier]
        total_amt = (base_price * loads) + selected_addon + open_amt
